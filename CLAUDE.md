# CLAUDE.md — Remotion Long-form Video Template

Template Remotion cho video dài (YouTube ngang) phong cách **"A colorful,
3D embossed hand-painting style with raised textures and bold brushstrokes"** theo hồ
sơ thương hiệu kênh LẠ ĐỜI NHẤT.
Layout: **2560×1440 (16:9, ngang)**, nhiều mốc sự kiện, text & icon nổi màu sắc ấn tượng.
...
7. **Edit video** — tạo/tinh chỉnh hiệu ứng chuyển cảnh mượt mà, đẹp mắt
   ...
   **Khi có khoảnh khắc cần "card text" (thẻ tiêu đề/trích dẫn giữa video)
   hoặc "chữ 3D"** — dùng 2 loại effect đã có sẵn trong `EFFECTS`
   (`src/Effects.tsx`), triển khai từ
   [`Du-lieu-lam-video/ho-so-ky-thuat-hieu-ung-chu.md`](Du-lieu-lam-video/ho-so-ky-thuat-hieu-ung-chu.md):
   - `{ type: "card", variant: "impact" | "cinematic" | "neon" | "foil", startSec, title, subtitle?, color? }`
     (`src/effects/CardText.tsx`) — 4 kiểu thẻ tiêu đề: đập vào kiểu Hồng
     Kông, mờ dần hiện ra kiểu Hollywood, neon nhấp nháy, chữ dát ánh kim
   - `{ type: "text3d", startSec, text, color?, fontSize? }`
     (`src/effects/EmbossedText3D.tsx`) — chữ nổi khối 3D embossed đúng
     phong cách kênh (thay cho kỹ thuật Metal/Ice Extrude bằng Three.js
     trong hồ sơ gốc — không cần cài thêm package hay chuẩn bị asset ngoài)
   - Không truyền `color`/`highlightColor` thì tự lấy `VIDEO_CONFIG.accentColor`
     / Data Yellow — **không tự ý dùng lại màu ví dụ gốc trong hồ sơ**
     (đỏ/vàng kim Hồng Kông, neon hồng...), luôn theo đúng bảng màu thương
     hiệu của video đang làm.
...
- Ảnh sinh ra ở khung ngang **2560×1440 (16:9)** — mô tả bố cục theo chiều
  ngang (chủ thể có thể lệch trái/phải, chừa khoảng trống hai bên/dưới cho
  text overlay).
...
- **Luôn gắn đúng video style, nguyên văn**:
  ...
  — kèm nền tối Obsidian `#0A0A0F` + màu nhấn thương hiệu, horizontal **16:9**
  landscape framing, kèm `no text, no subtitles`

**Dùng output:**
1. Mở [studio.google.com/flow](https://studio.google.com/flow) → chọn VEO 3, tỉ lệ khung hình **16:9**
...
(khung ngang 2560×1440, phong cách "A colorful, 3D embossed hand-painting style
...
src/
  ...
  Effects.tsx         ← EFFECTS array (8 loại hiệu ứng, điền tay theo scene)
  effects/
    CardText.tsx       ← "card" — 4 kiểu thẻ tiêu đề (impact/cinematic/neon/foil)
    EmbossedText3D.tsx ← "text3d" — chữ nổi khối 3D embossed phong cách kênh
...
public/
  ...
  sfx/                  ← SFX files
  images/bg/            ← bg-00.jpg, bg-01.jpg, ... (2560×1440, colorful 3D embossed hand-painting, 1 ảnh/scene)
...
Du-lieu-lam-video/
  ...
  ho-so-ky-thuat-hieu-ung-chu.md   ← hồ sơ card text/chữ 3D — đã triển khai
                                       thành src/effects/ (EFFECTS "card"/"text3d") — KHÔNG xoá
...
- Composition (`src/Root.tsx`) cố định **2560×1440** — không đổi sang dọc
